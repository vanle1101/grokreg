"""Claude / Anthropic registration plugin — runs sibling folder ../claude/main.py."""
...
class ClaudeToolPlugin(BaseToolPlugin):
    meta = ToolMeta(
        id="claude",
        name="Claude / Anthropic",
        description="Đăng ký Claude — OTP email → bắt sessionKey / cookie",
        icon="✦",
        status="ready",
        color="#D97757",
        fields=[
            ...
            ToolField(
                ...
                default="protocol",
                options=[
                    FieldOption("protocol", "HTTP không Chrome", "auth API + OTP code"),
                    FieldOption("auto", "Tự động", "HTTP rồi Chrome nếu fail"),
                    FieldOption("browser", "Chrome ẩn", "mở giao diện claude.ai"),
                ],
            ),
        ],
    )
    ...
    @staticmethod
    def claude_root(root: Path) -> Path:
        return root.parent / "claude"
    ...
    def preflight(self, params: dict[str, Any], root: Path) -> None:
        cl = self.claude_root(root)
        if not (cl / "main.py").exists():
            raise RuntimeError(f"Thiếu tool Claude: {cl}")
    ...
    def cwd(self, root: Path) -> Path:
        return self.claude_root(root)

    def stop_signal(self, root: Path) -> None:
        stop = self.claude_root(root) / "data" / "STOP"
        stop.parent.mkdir(parents=True, exist_ok=True)
        stop.write_text("stop:web\n", encoding="utf-8")
        try:
            import sys

            cl = str(self.claude_root(root))
            if cl not in sys.path:
                sys.path.insert(0, cl)
            from claudereg.stop import request_stop
    ...
    def parse_results(self, root: Path, limit: int = 200) -> list[dict[str, Any]]:
        path = self.claude_root(root) / "data" / "accounts.txt"
        if not path.exists():
            return []
