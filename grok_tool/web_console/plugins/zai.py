...
class ZaiToolPlugin(BaseToolPlugin):
    meta = ToolMeta(
        id="zai",
        name="Z.ai",
    ...
    @staticmethod
    def zai_root(root: Path) -> Path:
        return root.parent / "zai"
    ...
    def preflight(self, params: dict[str, Any], root: Path) -> None:
        z = self.zai_root(root)
        if not (z / "main.py").exists():
            raise RuntimeError(f"Thiếu tool Z.ai: {z}")
    ...
    def cwd(self, root: Path) -> Path:
        return self.zai_root(root)

    def stop_signal(self, root: Path) -> None:
        stop = self.zai_root(root) / "data" / "STOP"
        stop.parent.mkdir(parents=True, exist_ok=True)
        stop.write_text("stop:web\n", encoding="utf-8")
        try:
            import sys

            z = str(self.zai_root(root))
            if z not in sys.path:
                sys.path.insert(0, z)
            from zaireg.stop import request_stop
    ...
    def parse_results(self, root: Path, limit: int = 200) -> list[dict[str, Any]]:
        path = self.zai_root(root) / "data" / "accounts.txt"
        if not path.exists():
            return []
