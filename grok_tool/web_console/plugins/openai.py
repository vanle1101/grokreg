"""OpenAI / ChatGPT registration plugin — runs sibling folder ../openai/main.py."""
...
class OpenAIToolPlugin(BaseToolPlugin):
    meta = ToolMeta(
        id="openai",
        name="ChatGPT / OpenAI",
        description="Đăng ký OpenAI — OTP email → bắt session / token",
        icon="◎",
        status="ready",
        color="#10A37F",
        fields=[
            ...
            ToolField(
                ...
                default="protocol",
                options=[
                    FieldOption("protocol", "HTTP không Chrome", "auth0 / arkose solver"),
                    FieldOption("auto", "Tự động", "HTTP rồi Chrome nếu fail"),
                    FieldOption("browser", "Chrome ẩn", "mở chatgpt.com"),
                ],
            ),
        ],
    )
    ...
    @staticmethod
    def openai_root(root: Path) -> Path:
        return root.parent / "openai"
    ...
    def preflight(self, params: dict[str, Any], root: Path) -> None:
        oa = self.openai_root(root)
        if not (oa / "main.py").exists():
            raise RuntimeError(f"Thiếu tool OpenAI: {oa}")
    ...
    def cwd(self, root: Path) -> Path:
        return self.openai_root(root)

    def stop_signal(self, root: Path) -> None:
        stop = self.openai_root(root) / "data" / "STOP"
        stop.parent.mkdir(parents=True, exist_ok=True)
        stop.write_text("stop:web\n", encoding="utf-8")
        try:
            import sys

            oa = str(self.openai_root(root))
            if oa not in sys.path:
                sys.path.insert(0, oa)
            from openaireg.stop import request_stop
    ...
    def parse_results(self, root: Path, limit: int = 200) -> list[dict[str, Any]]:
        path = self.openai_root(root) / "data" / "accounts.txt"
        if not path.exists():
            return []
